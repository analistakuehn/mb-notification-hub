...
6. **Eval coverage** when the skill (a) declares `## Auto-Clarity`, (b) performs side effects, or (c) orchestrates other skills, agents, workers, or commands: ship `framework/evals/skills/<name>/eval.json` per `./.agents/araia/evals/README.md`. Purely informational skills are exempt.
