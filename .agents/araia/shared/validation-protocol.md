...
After an `F-BUDGET` termination, validation may run against files already persisted in staging. Apply the same structural and content checks above plus the expected-roster, referential-integrity, placeholder, hash, and next-step checks from `./.agents/araia/shared/durable-candidate-recovery.md`. A candidate that passes recovery validation is not re-generated; it continues at its recorded review, consolidation, approval, or gate-preparation step.
