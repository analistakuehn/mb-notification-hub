...
```
PT-BR output: read and apply `./.agents/araia/shared/ptbr-generation-rules.md` (do not paraphrase; load the file).
EN output: read and apply `./.agents/araia/shared/en-generation-rules.md` (do not paraphrase; load the file).
```
...
Read each rule file at most once per context window: when `ptbr-generation-rules.md` or `en-generation-rules.md` is already in the session context, apply it from context; do not Read it again (every re-read re-injects the whole file for zero new information). In the MAIN conversation, the headline summary in the project `CLAUDE.md` managed block plus the harness backstop below replace pre-emptive full-file reads. A full rule-file read belongs to: a dispatched agent (each dispatch is a fresh context and MUST load the file, per the Language section above), a dedicated language review pass, or resolving a hook finding that needs the canonical wording.

## Harness backstop (deterministic, covers subagents)

The master hook lives at `./.agents/araia/hooks/post-write-language-check.mjs`. `/araia init` and `/araia sync` copy it into each project's harness directory and register it as a `PostToolUse` hook. Claude Code executes the project copy through `Write|Edit|MultiEdit`. Codex matches `apply_patch|Edit|Write`, while Kimi matches `Write|Edit`; both execute the stable framework installation, and the Kimi command acts only when the project ledger is present.
