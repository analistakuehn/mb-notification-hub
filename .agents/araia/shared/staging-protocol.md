...
When a dispatch ends with `F-BUDGET`, apply `./.agents/araia/shared/durable-candidate-recovery.md` after the completion or kill notification and before retrying, pausing, or marking the stage failed. Validate the staging roster with the normal structural and content invariants. Continue from a `complete-valid` candidate's recorded next step; preserve valid units from `partial-valid`; never delete staging or regenerate validated units because of budget exhaustion alone.
