...
Skills that stage files MUST list the files by path. Wildcard staging risks pulling in `.env`, credentials, build artifacts, IDE config, or in-progress work the user did not mean to commit. The Bash-tool default rule in `~/.claude/CLAUDE.md` already says this for ad-hoc commits; this protocol extends the rule to every commit-producing skill.
...
- `~/.claude/CLAUDE.md`: base Bash-tool git safety protocol; this file extends it to skill/agent behavior.
- `./.agents/araia/shared/auto-clarity-protocol.md`: the trigger framework this protocol plugs into.
- `./.agents/araia/shared/failure-taxonomy.md`: hook failure maps to `F-TOOL`, but recovery routes through this protocol's Rule 1 (investigate, do not bypass).
- `./.agents/araia/shared/command-policy.md`: the mechanical, per-harness enforcement counterpart to Rules 1, 2, 3, and 6 above (Claude Code `permissions.ask` plus a `PreToolUse` hook; Codex `.rules`). This protocol remains the source of truth for *why*; that file and `command-policy.json` are the *what gets checked before the command runs*, where a harness can express it.
