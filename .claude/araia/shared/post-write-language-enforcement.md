...
```
PT-BR output: read and apply `./.claude/araia/shared/ptbr-generation-rules.md` (do not paraphrase; load the file).
EN output: read and apply `./.claude/araia/shared/en-generation-rules.md` (do not paraphrase; load the file).
```
...
The master hook lives at `./.claude/araia/hooks/post-write-language-check.mjs`. `/araia init` and `/araia sync` copy it into each project's harness directory and register it as a `PostToolUse` hook. Claude Code executes the project copy through `Write|Edit|MultiEdit`. Codex matches `apply_patch|Edit|Write`, while Kimi matches `Write|Edit`; both execute the stable framework installation, and the Kimi command acts only when the project ledger is present.
