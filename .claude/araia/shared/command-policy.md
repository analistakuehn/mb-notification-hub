...
|---|---|---|
| Claude Code | `.claude/settings.json` `permissions.ask`, one `Bash(<pattern> *)` entry per rule | `hooks/pre-bash-git-hygiene-check.mjs`, wired as `hooks.PreToolUse[]` matcher `Bash` |
