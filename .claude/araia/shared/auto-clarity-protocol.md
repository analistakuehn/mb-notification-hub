...
`./.claude/araia/shared/unknown-information-policy.md`.
...
- For agent outputs that cannot reach a finding at all: emit the structured uncertainty envelope from `./.claude/araia/shared/agent-uncertainty-protocol.md` instead of forcing a finding.
...
Every time a trigger fires and the skill blocks, asks, or pauses, the skill MUST append an entry to `.araia/refusal-log.jsonl` per `./.claude/araia/shared/refusal-log-protocol.md`. This is how the framework measures trigger calibration. Skipping the log silently is a protocol violation: the gate fired but no one can see it later. Append the log entry after capturing the user's resolution, so the entry can record the resolution; for non-interactive refusals (the skill aborts without a prompt), log immediately with `user_resolution: "n/a"`.
