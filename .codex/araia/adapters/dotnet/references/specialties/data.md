...
5. **Rule conflict**: proposed change violates `persistence` or `cache` axis in `.araia/stack-profile.yaml`, an existing ADR (e.g., persistence-per-bounded-context), or `~/.codex/AGENTS.md`. Cross-context queries bypassing module ownership require ADR-level authorization.
