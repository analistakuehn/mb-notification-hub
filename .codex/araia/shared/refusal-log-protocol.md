...
- `./.codex/araia/shared/auto-clarity-protocol.md`: the source of triggers `auto-clarity-1` through `auto-clarity-5`.
- `./.codex/araia/shared/git-hygiene-protocol.md`: the source of `git-hygiene-*` triggers.
- `./.codex/araia/shared/validation-protocol.md`: the source of `validation-*` triggers (C1 unsourced findings, S1 missing markers).
- `./.codex/araia/shared/agent-uncertainty-protocol.md`: agents that emit the uncertainty envelope log a `uncertainty-insufficient-evidence` entry.
- `./.codex/araia/shared/failure-taxonomy.md`: `F-GATE` failures map to `verdict-*` and `gate-*` triggers.
