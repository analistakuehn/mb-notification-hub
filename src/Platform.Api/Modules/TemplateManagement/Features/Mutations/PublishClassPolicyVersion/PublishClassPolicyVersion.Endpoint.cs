...
internal static partial class PublishClassPolicyVersion
{
    internal static void MapEndpoint(RouteGroupBuilder group)
        => group.MapPost("/publish", HandleHttpAsync)
        ...
        CancellationToken cancellationToken)
    {
        ...
        Result<Outcome> result = await handler.HandleAsync(application, @class, actor.Value!, cancellationToken);
        if (result.IsFailure)
        {
        ...
        return result.Value switch
        {
            ...
            Outcome.Blocked blocked => ApiResults.Problem(
                ...
                "The class policy validation blocked this publication. "
