...
{
    [RequiresDockerFact]
    public async Task The_migration_provisions_partitions_for_the_current_month_and_the_two_following()
    {
        await fixture.ExecuteDbAsync(async db =>
        {
            ...
            {
                partitions.ShouldContain($"templatemanagement.{MonthPartitionName(now, offset)}");
            }
        });
    }
    ...
    public async Task A_governed_effect_lands_its_audit_event_in_the_partition_of_the_occurrence_month()
    {
        ...
        await fixture.ExecuteDbAsync(async db =>
        {
            ...
            partition.ShouldBe($"templatemanagement.{MonthPartitionName(DateTimeOffset.UtcNow, 0)}");
        });
    }
    ...
    public async Task The_partition_manager_recreates_a_missing_future_partition_and_a_second_run_changes_nothing()
    {
        await fixture.ExecuteDbAsync(async db =>
        {
            ...
            var drop = $"DROP TABLE IF EXISTS templatemanagement.{futurePartition}";
            ...
            (await ListPartitionsAsync(db)).ShouldContain($"templatemanagement.{futurePartition}");
        });
    }
        ...
        => await db.Database
            .SqlQuery<string>(
                $"""
                 ...
                 FROM pg_inherits
                 WHERE inhparent = 'templatemanagement.audit_event'::regclass
                 """)
            .ToListAsync();
