...
| `SEC-014` | `MEDIUM` | **RESOLVIDO** | A trilha grava relatório completo na publicação e um booleano no... |
...
| **estado** | **RESOLVIDO** |
| nota de estado | Fechado, e a sondagem corrigiu a ficha em quatro pontos, refutou um eixo inteiro e mostrou que o alcance de hoje é maior que o registrado. **Correções.** São **onze** caminhos que gravam trilha, não cinco, e a razão em texto livre está em **quatro** deles, não em um. O eixo de **tamanho está refutado**: o relatório não cresce com o número de variáveis, e 5 ou 40 delas produzem exatamente os mesmos 1.130 bytes; o que faz o documento crescer é a quantidade de **avisos**, com pior caso medido de 15.610 bytes para 100 avisos. Como o próprio código declara publicações na casa das dezenas por mês, a economia de bytes é ruído e não sustenta o achado; ela é subproduto, e o commit não a vende como motivação. A recomendação da ficha, aplicada como escrita, **gravaria campo morto**: ela pede contagem e nomes de checks reprovados, e nos cinco caminhos de publicação e rollback esses campos são vazios por construção, porque relatório reprovado retorna bloqueado antes de a entrada de trilha ser montada. Onde a ficha pede o eixo de reprovação, a correção gravou o eixo de **aviso**, que é o que de fato varia ali. E o vazamento de host **não ocorre na publicação de template**, como a ficha afirma: lá as mensagens que nomeiam host vêm de check reprovado, que bloqueia e não gera evento. Ele ocorre na publicação de **layout**, como aviso, e esse aviso nasceu na correção de `SEC-005`, ou seja o alcance é maior hoje do que era quando a ficha foi escrita. **O que se confirmou**, e é o mecanismo: `Passed` é definido como nenhum check reprovado, então **aviso não reprova**, e um relatório aprovado chega à trilha carregando mensagem que interpola nome de variável e, no caminho de layout, host extraído do corpo. Confirmado por execução nos dois casos. O gatilho está inteiramente sob controle do autor: declarar variável e não usá-la basta. **Um achado que muda a premissa da recomendação**: a ficha supõe que o relatório completo continua disponível pelo endpoint de validação, e isso é falso em dois dos cinco caminhos. Provado com a mesma versão imutável e o mesmo hash de conteúdo: aprovada na publicação e reprovada na revalidação de hoje, porque o check de referência de layout lê estado mutável da identidade e da versão do layout. E para política de classe **não existe endpoint de validação nenhum**, de modo que o relatório da trilha é o único que jamais existiu para aquele ato; isso foi aberto à parte. Por isso a forma compacta guarda os **nomes dos checks que rodaram**: é o único campo que revalidar nunca reproduz, porque o catálogo é código, muda por deploy e não carrega carimbo, e dois achados desta mesma sequência acrescentaram checks a ele. Sem essa lista, a linha não diz contra qual conjunto de regras a versão foi aprovada. Guarda também quais checks avisaram e quantas vezes, porque a decisão de publicar por cima de um aviso é auditável e precisa deixar rastro, e `SEC-005` registrou por escrito que o autor pode publicar por cima dele. **A assimetria fechou subindo o rollback até a forma compacta, não descendo a publicação até o booleano**: aprovado sozinho não diz qual catálogo rodou nem que houve aviso, e é justamente o que nada mais no sistema guarda. A razão virou código canônico mais nota livre opcional, no precedente que a remoção de supressão já usa neste sistema, e o motivo de não colapsar num código só é que a trilha é o **único** lugar onde a razão existe: nenhum agregado a recebe, nenhuma coluna a guarda, a resposta não a ecoa. O vocabulário inclui um verbete de escape que exige nota, e isso não é conveniência: a desativação é interruptor de emergência, e recusar uma parada de tráfego por falta de verbete é pior que a ambiguidade que a lista remove. O que motivou o vocabulário não foi estética: a razão em prosa vira **nome de categoria em relatório mensal de conformidade arquivado**, porque a leitura de evidência agrupa por ela e a composição a copia verbatim, de modo que cada sentença distinta cria um balde e os contadores por motivo não contam nada; e módulos irmãos já gravam código canônico na mesma coluna e no mesmo agrupamento. Como **nada impunha a regra** (não há teto de tamanho na entrada de trilha, não havia teste de arquitetura sobre os detalhes, e o contrato publicado de auditoria é mais frouxo que o do módulo), o que impede a regra de voltar a ser prosa é um teste novo de arquitetura de segurança que varre os produtores de detalhes e reprova quem referenciar mensagem ou localização de check. O contrato do módulo foi desambiguado no mesmo movimento, porque a frase antiga era ambígua entre nome e valor de variável, e essa ambiguidade foi o que permitiu as duas leituras. O que permanece aberto: **não há alcance retroativo**, e é impossível por construção, porque o gatilho de somente inserção recusa correção e os detalhes entram canonizados dentro do documento encadeado por hash, então reescrever quebraria a cadeia da partição; as linhas anteriores seguem com relatório completo, com nome de variável e host, e com razão em prosa, e o relatório mensal vai mostrar sentenças até o corte e códigos depois. O corte ficou registrado por discriminador intrínseco e não por data, porque a data seria afirmação sobre quando isto entra em produção. E a frase paralela do contrato, na seção de log, segue com a mesma ambiguidade entre nome e valor, sobre outra superfície e sem medição própria. |
