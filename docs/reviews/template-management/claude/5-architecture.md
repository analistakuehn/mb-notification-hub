...
| `ARC-008` | `LOW` | **RESOLVIDO** | Os cinco efeitos de layout não preenchem `Application` na trilha |
...
| **estado** | **RESOLVIDO** |
| nota de estado | **A JUSTIFICATIVA DA FICHA PARA A SEVERIDADE ESTÁ MEIO FALSA, E A METADE FALSA É A PIOR.** Ela sustenta `LOW` dizendo que *'depreciar ou desativar um layout não quebra render existente, já que a resolução do wrapper carrega a versão fixada sem consultar o status'*. Medido: **depreciar** está certo, e **desativar está errado**. O resolvedor **consulta** o status da identidade (`PublishedTemplateRenderer.cs:353`, `FramesMessages` é `value != Disabled`), e o comentário do próprio código diz o que a ficha nega, que a desativação *'answers no for every template that pins it'*. O que o resolvedor **não** consulta é o status da **versão fixada**, e é por isso que criar, depreciar, publicar versão e reverter de fato não tocam render existente: são quatro dos cinco, não cinco. **O CRUZAMENTO QUE A FICHA NÃO TEM, e é o que torna o achado pior:** a única transição de layout que corta disponibilidade para toda aplicação que fixa aquele layout é **precisamente uma das cinco cuja linha de trilha não diz para quem cortou**. A perda não é 'atribuição, nunca disponibilidade': é atribuição **incluindo a de um evento de indisponibilidade**. **DECISÃO DE ARQUITETURA, que a ficha pede: layout É recurso global por desenho**, e a razão que fecha é mais forte que os três sinais estruturais (agregado com time dono e sem aplicação, rota de topo em vez de sob `/v1/applications/{app}/`, e pino que não exige relação nenhuma): **o campo é de valor único e o recurso é muitos-para-muitos**. Um layout fixado por templates de cinco aplicações **não tem uma aplicação**, e preencher exigiria escolher arbitrariamente ou gravar cinco entradas para um efeito. **A omissão não é descuido, é o único valor honesto**, e por isso a segunda alternativa da ficha (acrescentar aplicação ao agregado) foi **recusada**. **SEVERIDADE: `LOW` sobrevive, e a justificativa é SUBSTITUÍDA.** Não sustenta mais 'o efeito não é de runtime', que é falso. Sustenta o **caminho alternativo de investigação**, verificado perna por perna antes de gravar: a recusa loga no ponto da recusa com chave e número do layout (`DisabledLayoutRefused`, evento 2111); `layout-disabled` está no catálogo canônico e o mapa do consumidor o **preserva por igualdade** em vez de colapsar no genérico, então chega ao produtor; e a consulta da trilha por chave de layout é **indexada** (`ix_audit_event_entity` sobre `(entity_type, entity_id)`), enquanto o filtro por aplicação **não tem índice nenhum**. Ou seja, quem investiga **não fica cego, fica sem o atalho**. **PRECISÃO QUE O IMPLEMENTADOR EXIGIU E QUE EU NÃO TINHA:** nenhuma das duas consultas existe como **caminho de código**; as leituras de trilha andam por janela de tempo e por sequência, e a diferença entre elas é de **armazenamento, não de rota**. Ficou escrito para ninguém prometer um relatório que não existe. E segue valendo que **não existe leitura de trilha filtrada por aplicação em lugar nenhum**, então a perda não é observável por caminho de código hoje: ela morde quem escrever a primeira consulta assim, ou quem investigar direto na tabela por que os templates de uma aplicação pararam de renderizar. **ENTREGUE**: o registro da decisão, e um portão que **protege a decisão do conserto errado**, que é preencher o campo com uma aplicação arbitrária e fazer a trilha mentir com aparência de completude. Escrito **sem lista de exceção**, ancorado em `typeof(...).Namespace` e nunca em string, com igualdade bilateral, tripwire nos dois lados, e cruzamento de descoberta entre metadado e texto de fonte. Precisou de **caminhada de IL** com BCL pura e zero pacote novo, porque a ferramenta de arquitetura resolve dependência no nível de tipo e não distingue a atribuição do campo. **O escopo é o módulo por MEDIÇÃO e não por preferência**: solução inteira exigiria lista de exceção, porque dois escritores de trilha fora do módulo omitem o campo por razão alheia. **A contagem da ficha e a minha estavam incompletas**: há um **sétimo** sítio que atribui, numa rotina de retenção, então são doze no módulo, cinco omitindo e sete atribuindo, e ele cai no lado certo **fortalecendo** o portão. Quatro mutações, todas nascendo na asserção e nunca no compilador, e a do tripwire imprimiu os doze nomes, que é confirmação independente da contagem. **O implementador PAROU antes de gravar** quando a minha afirmação caiu, em vez de escrever no registro durável uma frase que a medição dele contradizia, e pediu aprovação da redação corrigida. Portões verificados de forma independente: build com 0 erros e 0 avisos, ArchTests **30** contra base de 27, SecurityArchTests 14, UnitTests 1707. |
