...
Os sete últimos motivos entraram no catálogo na fase 1b porque já eram produzidos pelo código aceito, na ingestão REST e no pipeline, e mapeá-los para `payload-invalid` destruiria exatamente o diagnóstico de que o produtor precisa para corrigir a solicitação. O mesmo valor é o `type` do problema RFC 9457 na resposta REST e o `reason` do evento no barramento: um vocabulário só, qualquer que seja o transporte. `producer-disabled` permanece **inalcançável na fase 1b**: o `PRODUCER_REGISTRY` não tem coluna de habilitação, por decisão, e o corte de um produtor é o kill switch mais a ACL do broker; o valor fica declarado para que o vocabulário não mude quando o kill switch chegar. `quiet-hours` não pertence a este catálogo: adiamento não é recusa.
