...
|---|---|---|
| **Template** | Identidade e metadados governados: `key`, `application`, `class`, `owner_team`, `purpose`, `legal_basis`, `links_allowed` | Metadados que afetam governança (classe, base legal) só mudam via nova versão publicada; status `active \| deprecated \| disabled` |
| **Versão** | Unidade de publicação. Carrega `variables_schema` (JSON Schema), `sensitive_variables`, `change_note` (opcional), `layout_version` fixada, `content_hash` agregado | **Imutável após publicada**; qualquer alteração = nova versão. Rascunho editado com `PUT` idempotente e `ETag`/`If-Match` |
...
- Na v1, templates cuja **versão publicada** declara `sensitive_variables` não vazias (OTP, código de confirmação, token) **só aceitam solicitação via REST**. Evento Kafka para um template assim → `.dlt` + `NotificationRejected(reason = sensitive-variables-on-bus)` + `audit_event`. O envelope de cifra para variáveis sensíveis no barramento fica fora da v1.
- A lista pertence à **versão publicada** e não à identidade, porque ela é objeto de aprovação e a máquina de quatro olhos é toda de versão: hash, `approval` e imutabilidade após publicada existem na versão e não existem no template. Para o produtor a regra não oscila, porque a publicação **recusa a versão que larga uma variável sensível em vigor**: uma vez declarada, a lista só cresce, e a restrição de transporte, uma vez ligada, não desliga.
- A recusa depende **apenas de a versão publicada declarar variáveis sensíveis**, jamais de a variável estar presente no payload. É o que torna a regra decidível pelo produtor antes de publicar e o que impede o próprio controle de inspecionar exatamente os valores que existe para proteger. Pela mesma razão, a checagem roda **antes** da validação de variáveis contra o esquema: a validação produz um relatório de verificações sobre o payload, e esse payload é justamente o que não deve ser inspecionado nem descrito.
...
- **A coluna "Quem (Entra)" descreve como os principais são organizados no provedor de identidade, não o que o hub impõe.** O hub impõe papel na rota, mais quatro olhos no recurso para publicação e rollback, e nada além disso: não existe escopo por `application` na superfície de gestão. Qualquer portador de `Templates.Author` lê, cria e edita rascunho de template e de política de classe de **qualquer** aplicação; qualquer portador de `Templates.Publish` publica, deprecia, desativa e faz rollback de template de **qualquer** aplicação. Ver o risco 28 do §16.
...
| 27 | **Um comando de parada de template ou de layout alcança a LEITURA publicada da frota em 60 segundos mais a leitura que já estava em voo no commit, e esse excedente não tem teto** (desativação e depreciação de template, desativação e depreciação de layout, publicação de versão, rollback e publicação de política de classe) | **Errata de 2026-08-31, medida.** A redação anterior dizia "em até 60 segundos" e estava incompleta na direção perigosa. Medido: o excedente tem mediana 0,228 ms e p90 0,406 ms, e chega a 30 s sob inanição do pool de threads, então o limite típico é 60,0002 s e o patológico fica perto de 90 s. A causa não é conserto de cerca: a cerca de geração fecha a corrida intraprocesso entre carga em voo e invalidação local, e o worker não tem invalidação local para cercar, porque os handlers que invalidam são compostos só pelo host de API. **Segunda correção: a redação anterior tratava leitura e tráfego como a mesma coisa.** Entre a decisão de render e o envio existe a fila de dispatch, que estes ponteiros não governam, e uma mensagem renderizada sob ponteiro de 59 s e despachada muito depois sai de qualquer forma. **Decisão de aceitação, revista e mantida com o limite honesto.** Encurtar a janela pagando `K × P × 6 / W` fica **recusado**, porque é a alavanca errada por medição: a fila drena de qualquer jeito e apertar a leitura não para tráfego. Quem precisar parar um template em menos de 60 segundos precisa de controle no caminho de despacho, onde o kill switch já age e onde hoje existem escopos de produtor, aplicação e canal, e nenhum de template. Esse caminho fica **nomeado e não escolhido aqui**, porque muda de módulo e é decisão de propriedade de contexto. **Achado de capacidade, do mesmo registro:** o orçamento de 4096 entradas de ponteiro é um só para todas as famílias de chave portadoras de decisão, e um template quente ocupa duas; medido, 1.800 quentes respondem 0,45% das leituras do cache e 2.200 respondem 7,65%, logo 7% a mais de conjunto compra 15 vezes mais consultas e o joelho fica **abaixo de 2.048**, não em 4.096. Nada testemunha a travessia |
| 28 | **A superfície de gestão não impõe posse por `application`: qualquer publicador desativa, deprecia ou faz rollback do template de produção de outro time, e qualquer autor lê e edita rascunho alheio, inclusive o de política de classe** | Nada vincula um principal a uma `application` nesta fase, e o mesmo vínculo ausente já responde pelas pendências 25 e 37 da fase 1b, com dono Arquitetura e Segurança. O que contém no lugar do escopo: os dois papéis são atribuídos por grupo restrito no Entra (§9.1); a superfície é interna, alcançável só por rede interna ou ZTNA (§10.2 A6); toda transição grava `audit_event` transacional; e o quatro olhos impede ato unilateral na publicação. **O que a contenção não cobre**: a trilha registra quem fez, não que faltava autoridade, e `deprecate` e `disable` são unilaterais, sem quatro olhos, com efeito terminal sobre tráfego. A decisão de impor escopo está amarrada à forma do vínculo principal para `application` da pendência 25, porque as três superfícies dependem do mesmo vínculo |
