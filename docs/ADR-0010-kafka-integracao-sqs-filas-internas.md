...
- **Variáveis sensíveis**: na v1, templates com `sensitive_variables` só aceitam solicitação via REST. Evento Kafka com variável sensível → `.dlt` + rejeição (`sensitive-variables-on-bus`) + `audit_event`. O envelope de cifra para variáveis sensíveis no barramento fica fora da v1 (ADR futura prevista). Retenção do tópico de entrada em 24 h; ACL de leitura restrita ao consumer group do hub.
