...
1. **Banco, append-only por construção.** `audit_event` (e `consent`, `approval`, `template_version`, `class_policy_version`, `delivery_event`, `delivery_payload`) com role de aplicação que só tem `INSERT`/`SELECT`; trigger `BEFORE UPDATE OR DELETE` que lança exceção; partições antigas protegidas por `REVOKE` de escrita + trigger de bloqueio (PostgreSQL não tem modo read-only por tabela); `pgaudit` ativo. O `audit_event` é gravado **na mesma transação** do efeito que registra (aceite, decisão de política, tentativa, entrega, consentimento, publicação, configuração, leitura de auditoria).
