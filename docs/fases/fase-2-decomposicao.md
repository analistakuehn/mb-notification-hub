...
| F2-3 | Pergunta 7 da reconstrução respondível | F2-2 | Concluída (commit `9c4cbb4`) |
...
**Riscos e armadilhas.** O predicado parcial precisa aparecer literalmente na consulta, senão o planejador ignora o índice, que é o defeito que custou uma rodada de medição na fase 1b. A liberação precisa transitar de `deferred` para `accepted` dentro da transação do claim: sem isso, `CoreMessageProcessor.cs:78` trata a retomada como duplicata e a notificação adiada nunca sai. A varredura soma o próprio intervalo ao prazo, contra um timeout de fallback de 30 s no plano de `critical`, e esse trade-off já está aceito no design.

**Nota posterior, da remediação da revisão da fase 2.** O padrão do intervalo passou de 5 s para 2 s. O motivo é aritmético e não preferência: somando o prazo do passo, o intervalo, os dois saltos de fila, o estágio Core e a chamada ao provedor, o valor anterior colocava o tempo até o SMS de fallback acima do aceite de 35 s do §11.6, e nenhum oráculo media isso. O timeout do provedor de SMS foi ajustado no mesmo movimento e uma asserção de orçamento passou a reprovar quando qualquer um dos dois sobe sozinho. A varredura por prazo também passou a alcançar tentativas em `queued`, porque circuito aberto, throttle e kill switch devolvem a tentativa à fila com o prazo intacto e nenhuma varredura a enxergava.
