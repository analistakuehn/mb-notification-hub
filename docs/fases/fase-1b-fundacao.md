...
| B10 | Kafka Ingress Worker (`notifications.requested.v1`, `.dlt`, `PRODUCER_REGISTRY`, §7.2), emissão de `notifications.events.v1` e publicador Kafka do outbox relay (§1.3) | B4, B5 | Em implementação |
...
| 29 | `failed.reason` viaja com o vocabulário de erro do despacho (código do provedor, `no-active-device-token`), não com o catálogo canônico de rejeição, e o §7.3 não distingue os dois | Definir o vocabulário próprio de falha de entrega, ou declarar explicitamente que `failed.reason` é livre e só `rejected.reason` é catálogo; decidir junto da API de consulta (B12), que expõe os dois; dono: Arquitetura com Engenharia | §7.3; fatia B10 (2026-08-23) |
