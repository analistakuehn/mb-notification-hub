...
| 20 | Cadência da verificação integral (semanal por partição aberta) foi escolhida sem medição do custo de releitura da partição corrente | **Curva medida antes do índice de cauda, e precisa ser remedida depois dele**: o verificador varre por `seq` e é provável que boa parte da superlinearidade seja a mesma ordenação sem índice do caminho quente, de modo que publicar o número de partição inteira como insumo de cadência congelaria um valor que o índice pode já ter invalidado. Curva medida em 2026-08-23, sem meta fixada porque a cadência depende de volume real de produção, que não existe: a releitura integral da partição corrente custou 0,48 s por 100 mil linhas com dez mil linhas, 0,77 s por 100 mil com quinhentas mil, e 4,28 s por 100 mil com dois milhões, ou seja 85,75 s para replicar a partição inteira de dois milhões. Realvada para pré-produção com a curva anexada; dono: Engenharia | §9.4; fatia B15 |
...
| 44 | Nenhum índice atende à consulta de cauda que roda dentro do lock em todo append, e a mesma ausência afeta o verificador horário e o export por faixa de `seq` | Achado da medição de 2026-08-23, com plano de execução lido sobre partição de dois milhões de linhas: sem índice a consulta custa 330 ms lendo 181.230 buffers; com índice parcial em `(occurred_at, seq DESC)` o planejador ignora o índice e repete a varredura em 371 ms; com índice parcial em `(seq DESC)` custa 0,174 ms lendo 4 buffers. A forma composta não resolve porque a poda de partição já satisfez o predicado de tempo, deixando a coluna da frente como prefixo inútil, e porque o predicado restante é faixa e não igualdade, de modo que a composta não fornece ordenação por `seq` dentro dela. Forma ratificada pelo architect em 2026-08-23: `(seq DESC) WHERE hash IS NOT NULL`, com duas notas de aplicação, o predicado parcial precisa aparecer literalmente na consulta para casar e aparece hoje, e o mesmo índice serve o verificador horário e o export por faixa de `seq`. Aplicação na fatia corretiva C2, medindo antes e depois; a sonda mede o índice em braço próprio e nunca o aplica; dono: Engenharia | §9.4; §11.3; fatia B15; fatia C2 |
