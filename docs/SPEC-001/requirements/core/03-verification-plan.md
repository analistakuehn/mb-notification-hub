...
| `VER-008` | `PAC-008`, `ER-008`, `ER-015`, `NFR-007` | Solicitações sem anexos preservam REST, Kafka, hash, recusas, eventos e seleção de canal. | Suíte brownfield e vetores dourados apresentam zero divergência antes e depois da mudança. Pré-condição: os vetores dourados são cunhados e congelados a partir do comportamento vigente antes da primeira alteração da forma canônica, porque depois dela o lado anterior deixa de ser reconstruível. | unitário, integração e contrato | `dotnet test MonteBravo.NotificationHub.sln --no-restore` | Clientes e mensagens sem o novo membro atravessam versões mistas. | CI, G5s, G6 | `Notifications` |
| `VER-009` | `PAC-009`, `ER-008` | Mesmo manifesto retorna replay; diferença relevante retorna conflito sem novo efeito. | Matriz cobre ordem, duplicatas e propriedades conforme o corpus aprovado; contadores duráveis permanecem estáveis. Pré-condição: o congelamento de vetores exigido por `VER-008` precede esta matriz. | unitário e integração | `dotnet test tests/Platform.UnitTests/Platform.UnitTests.csproj` | Hash, idempotência, claim e persistência real. | CI, G5s | `Notifications` |
...
| `VER-012` | `PAC-012`, `ER-007`, `ER-014`, `NFR-004` | Nenhuma superfície coletada contém conteúdo, localização ou capacidade de acesso. | Sentinelas únicas semeadas no arquivo não aparecem em Kafka, SQS, outbox, dead-letter, logs, traces, métricas, respostas ou auditoria comum. Pré-condição: a varredura com sentinelas é criada antes da primeira superfície que ela deve cobrir. | integração e segurança | `dotnet test tests/Platform.IntegrationTests/Platform.IntegrationTests.csproj` | Todos os transportes e coletores do cenário. | CI, G5s, G6 | Todos os contextos afetados |
...
| `VER-015` | `ER-001`, `ER-011` | Dependências respeitam propriedade dos contextos e contratos versionados. | NetArchTest reprova acesso a Domain, Infrastructure, EF ou S3 de outro contexto e dependência de `Dispatch` para anexos internos. Pré-condição: a regra vigente que mantém o domínio livre de tecnologia não lista `Amazon`, portanto a parte de S3 exige regra nova, provada por mutação antes de contar como oráculo. | função de adequação | `dotnet test tests/Platform.ArchTests/Platform.ArchTests.csproj` | Grafo de assemblies da API e do worker. | CI, G5s, G6 | Adapter `dotnet` |
...
| `VER-019` | `ER-015`, `NFR-007` | A versão contratual escolhida preserva consumidores antigos ou coexiste com a versão anterior. | Snapshots de OpenAPI e schema, serializadores e consumidores de versão anterior passam contra o produtor novo. Pré-condição: os snapshots são capturados do contrato vigente antes da primeira mudança de contrato. | contrato | `dotnet test tests/Platform.IntegrationTests/Platform.IntegrationTests.csproj` | REST, Kafka, dead-letter, eventos e `Integration.*`. | CI, G5s, G6 | `Notifications` e adapter `dotnet` |
| `VER-020` | `ER-016`, `NFR-008` | Buffer, streaming e spool são comparados com o mesmo corpus e envelope. | Relatório registra payload UTF-8, heap, working set, alocação, GC, CPU, I/O, latência, throughput, backlog, limpeza e igualdade de digest; a opção promovida respeita o orçamento aprovado. Pré-condição: o braço comparativo é acrescentado ao runner e à pasta de baselines existentes antes da promoção. | desempenho e capacidade | `dotnet run --project tests/Platform.PerformanceTests/Platform.PerformanceTests.csproj` | S3, serialização, fake HTTP, cancelamento e encerramento abrupto. | G5s, G6, rollout | `dotnet-architect` e `Dispatch` |
