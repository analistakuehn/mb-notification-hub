...
- **Depende de**: 27
- **Estado**: Done em 2026-09-03.
- **O caminho ficou vivo**: o conjunto chega ao provedor. O processador de despacho passou a submeter o conjunto no pedido, e é **o mesmo objeto que a revalidação verificou**, não uma segunda leitura da linha: ler duas vezes abriria janela para a linha mudar entre o veredito e a chamada. A mutação que troca esse objeto por uma cópia igual derruba exatamente um oráculo, o de identidade, porque cópia igual atravessa qualquer asserção que compare conteúdo.
- **A pergunta tem um dono só**: quem responde se o canal transporta o conjunto é o adaptador, como membro obrigatório do contrato e não propriedade com valor padrão, de modo que adaptador novo não compila sem decidir. O módulo dono publica a pergunta sobre o resolvedor que já existia, e a implementação não guarda resposta nenhuma: quem planeja e quem envia leem o mesmo objeto. Uma tabela por canal em qualquer outro lugar seria uma segunda afirmação, correta no dia em que fosse escrita e silenciosamente errada no dia em que um canal apontasse para outro adaptador.
- **A rota recusa e não reescreve**: o plano não é filtrado, reordenado nem reescrito, nenhum anexo é removido e nada vira link. Falha de resolução lança em vez de recusar, porque uma recusa ali diria ao produtor que a notificação foi negada por uma falha que é nossa; e só notificação que carrega conjunto chega a perguntar, então implantação sem anexo nunca é retida por isso.
- **O fallback termina e não caminha adiante**: elegibilidade é sobre o destinatário e transportar o conjunto é sobre a mensagem, então pular deixaria a presença de um anexo reordenar um plano publicado sem ele. A mesma palavra fecha os três pontos, alcançada por constante em vez de reescrita, para que quem lê uma tentativa falhada ao lado de uma notificação recusada não aprenda dois nomes para um fato. Uma guarda de última linha recusa a chamada quando o adaptador resolvido responde que não transporta, de modo que a propriedade vale sobre o objeto que faz a chamada e não apenas sobre quem planejou.
- **Consequência de produto, assumida**: e-mail é hoje o único canal que transporta o conjunto, então notificação com anexo passou a não ter fallback. Se o e-mail falha, ela termina, sem resgate por outro canal. É o que a terminação explícita significa, e está dito em vez de descoberto depois.
- **Nove mutações de um eixo, nenhuma voltou verde.** A que mais importava derruba um oráculo só: sem ele, ligar o conjunto ao envio teria ficado sem falsificador nenhum e as recusas medidas seriam regras sobre um caminho que não carrega nada.
- **Um vermelho falso ensinou a forma forte da regra**: depois de uma reversão byte a byte, com o build reportando sucesso, a suíte acusou exatamente o teste que a última mutação derruba. A reversão restaura o carimbo de tempo original, que fica mais antigo que o binário compilado sob a mutação, o MSBuild conclui que a saída está em dia e **não recompila**. A regra de não usar build incremental depois de reverter foi seguida e não bastou: **o código de saída do build não prova recompilação**, e o que prova é o carimbo do binário ser posterior ao da fonte.
- **Redução de cobertura declarada, não maquiada**: dois testes vizinhos perderam alcance porque uma notificação com conjunto não chega mais ao segundo passo do plano. As superfícies que esse passo escreveria ficaram fora do alcance da regra de não cópia, e isso está escrito no próprio teste.
- **Defeito de repositório corrigido de passagem**: o `.gitignore` engolia um diretório de código de produção, cujos dois arquivos existiam no disco e não no git, de modo que um clone limpo não construía o repositório. Medido: toda pasta com aqueles nomes está dentro de saída de build já ignorada, exceto justamente a de código, então os padrões foram removidos em vez de negados caso a caso. Negar consertaria este caso e deixaria a armadilha armada para o próximo diretório de mesmo nome.
- **O que os oráculos não provam**, declarado: nada sobre provedor real, cujos dois testes de fumaça seguem pulados por desenho; nada sobre os bytes exatos no braço de ponta a ponta, que compara nome, tipo e comprimento decodificado por membro, com a identidade dos bytes medida pela testemunha em outro lugar; nada sobre a corrida entre mudança de configuração de provedor e envio em andamento, porque a guarda é medida por substituição no contêiner; nada sobre o custo de o papel de rota hospedar adaptadores; e documento ilegível não é pergunta da rota, de modo que uma linha corrompida gasta uma tentativa enfileirada antes de encontrar a recusa que a detém.
...
- **Estado**: Done em 2026-09-03.
- **O que passou a convergir**: bytes duráveis sob uma chave que continua tomada, de modo que toda retentativa era recusada. Esse passivo só existia como linha de log. Uma rodada lista a chave derivada, remove o que o registro não reivindica e devolve a chave, medido como **uma geração antes e zero depois, com envio recusado antes e aceito depois, no mesmo anexo**. O veredito que não conclui ganhou palavra própria e é encerrado pela primeira rodada depois do prazo, por chamada à operação dona da máquina de estados, e não por transição escrita na rodada: uma transição própria seria uma segunda máquina de estados livre para concluir o que a primeira nunca concluiria.
- **Duas palavras, não indicador**: a rodada decide o reparo lendo a coluna, sem voltar ao armazenamento, ao registro e ao relógio para descobrir qual executar, que é justamente o custo que a coluna existe para evitar. Uma coluna só basta porque as duas palavras vivem em estados disjuntos: custódia só é devida enquanto os bytes nunca chegaram, veredito só depois que chegaram.
- **A coluna mora no agregado, e o fundamento fecha a alternativa**: um anexo que deve custódia é exatamente aquele que **não tem geração registrada**, então uma linha própria teria de ser inventada para a dívida, antes da escrita, que é a mesma razão pela qual a linha de intenção foi recusada.
- **A medição mais cara desta tarefa foi uma reprovação real, não uma mutação.** O gatilho no conflito de upload, que a entrada herdada nomeava como a assinatura observável do órfão, foi implementado e derrubou o teste de envios concorrentes. O agregado carrega versão de linha, então a anotação do perdedor mudava a versão entre a leitura e a gravação do vencedor; o vencedor passava a não casar linha alguma, respondia conflito e **compensava, removendo bytes que já tinha armazenado com sucesso**. Um envio legítimo morria por causa de um envio concorrente atrasado. O gatilho foi retirado, com oráculo dedicado segurando a forma, e a mutação que o devolve derruba dois testes.
- **Fronteira que vale além do caso**: enquanto o passivo mora na linha do agregado, nenhum caminho concorrente com um upload em voo do mesmo anexo pode anotá-la. Os dois gatilhos que sobraram são seguros porque a escrita condicional os torna exclusivos de qualquer upload que ainda possa vencer.
- **Preço declarado**: o órfão de amplificação de retentativa **não entra sozinho na fila**. A rodada sabe repará-lo e isso está medido; o que não existe é gatilho seguro que o registre a partir da requisição que encontra o conflito. A matriz traz essa linha explicitamente, com o desfecho inalterado.
- **Passivo deliberadamente fora**: a evidência de submissão pendente, porque nasce no módulo de despacho depois que os bytes saíram e **não tem reparo**, então materializá-la aqui faria um módulo escrever a linha de outro e encheria a fila de itens que ninguém pode reparar; o armazenamento inalcançável, que é a falha comum e nada afirma sobre bytes duráveis; e a recusa conclusiva de validação, que é estado final e não passivo.
- **O plano de consulta é evidência executada**: o comando foi capturado do próprio pipeline por interceptador, e não transcrito, e o `EXPLAIN` correu sobre ele com os mesmos parâmetros. Com o índice parcial, varredura de índice sem nó de ordenação; sem ele, varredura sequencial com ordenação por cima e custo estimado cento e vinte e três vezes maior. A metade da aceitação que fala em partição **não tem sujeito neste módulo**, porque a tabela de anexos não é particionada, e isso está dito em vez de apresentado como prova.
- **Treze mutações de um eixo, treze vermelhos, nenhuma verde.** Uma reversão foi **recusada pela ferramenta** por âncora ambígua e refeita com âncora única, em vez de escolher ocorrência: sem essa recusa a mutação seguinte teria empilhado sobre reversão parcial. Uma guarda foi **removida por não ter falsificador**, em vez de mantida como decoração.
- **Conserto de arranjo herdado**: as nove reprovações da reconciliação de entrega eram assimetria de relógio, com a fixture fixa em agosto e as partições provisionadas a partir do relógio de parede. O arranjo passou a provisionar as partições do próprio relógio, com a lista vinda da constante de produção e não de uma cópia, para que uma tabela particionada nova não deixe o arranjo para trás em silêncio. Nenhuma asserção afrouxada, nenhuma linha de produção tocada.
- **O que os oráculos não provam**, declarado: o órfão de amplificação sem gatilho é a maior lacuna e fica para a varredura de abandono de outra tarefa; a convergência é medida por anexo e não por contagem sobre a tabela, porque a suíte divide o banco com os vizinhos; e nada foi medido sobre o custo da rodada sob carga real.
...
- **Estado**: Done em 2026-09-04.
- **76 arquivos e 35 migrações viraram 21 arquivos e 7 iniciais**, uma por contexto. Foram sete e não seis porque o módulo de anexos, que não tinha cadeia alguma, ganhou a sua.
- **O congelamento saiu do código e entrou no banco.** Antes ele vivia só no mapeamento e recusava duas das quatro formas de reescrever a linha de geração; a atualização por conjunto e o SQL cru passavam e reescreviam o resumo durável em silêncio. Um gatilho de linha, antes de alteração ou exclusão e **sem cláusula condicional**, recusa as quatro. Isso fecha, no armazenamento, o buraco que a evidência reconstruível tinha declarado como limite dela.
- **Achado que corrige um entendimento anterior**: com a chave estrangeira mutada de restritiva para cascata, o anexo **continua indelével**, porque a cascata tenta apagar a filha e o gatilho recusa. Quem torna o anexo indelével é o **gatilho**, não a chave restritiva; a chave só decide qual erro chega. Um teste vizinho passou a medir as duas recusas separadamente, desligando o gatilho para alcançar a que ficava atrás.
- **Os arranjos de anexo passaram a migrar em vez de criar do modelo**, e eram **seis** e não três como o encaminhamento supunha. Enquanto criassem o esquema do modelo, o gatilho não existiria em teste e todo oráculo sobre ele seria vazio, porque a criação pela ferramenta cria zero gatilhos.
- **Cerimônia suspensa, e dito em vez de encenado**: tempo limite de bloqueio, ensaio de contenção no pai e numa partição e coluna anulável sem default existem para alterar tabela viva, e a decisão do dono os suspendeu até o primeiro DDL sobre tabela com dados em produção. A cláusula de três segundos na aceitação acima é anterior a essa decisão e está vencida por ela. Medir contenção de bloqueio sobre banco recém-criado passaria verde sem provar nada.
- **O que o esmagamento apagou**, listado: a conversão entre schemas, a adoção da trilha, o preenchimento da banda do outbox sobre linhas anteriores, a partição de mês anterior ao corrente, uma função órfã e três valores padrão que eram resíduo de alteração sobre tabela viva. Dois testes nomeavam migração, e cada um teve o **vermelho medido antes** da decisão de remover.
- **O esquema resultante foi comparado com o da cadeia antiga** por inventário de catálogo em duas bases descartáveis: 485 linhas de diferença com as migrações só geradas, 41 depois do SQL escrito à mão, todas classificadas.
- **Um teste foi apagado por medição, e o motivo é exemplar**: o construtor escreveu um teste para o outbox, ele passou, e a mutação de falsificação derrubou **ele e o irmão juntos, com a mesma lista de desacordos**. O irmão já cobria tudo. Um teste que nenhuma mutação distingue do vizinho não é cobertura nova, é duplicata com uma cópia livre para divergir.
- **Correção do orquestrador na mesma área**: a lista de tabelas particionadas estava escrita duas vezes e a cópia do arranjo de auditoria **já tinha ficado duas tabelas para trás** da constante de produção. Passou a derivar da constante, como o arranjo vizinho já fazia.
- **O que os oráculos não provam**, declarado: os métodos de reversão das migrações nunca foram executados; a comparação de esquema entre as duas cadeias não está automatizada; e a suspensão da cerimônia significa que nenhuma medição desta tarefa diz o que acontece sob DDL em tabela viva.
